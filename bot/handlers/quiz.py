...
from bot.content import DEFAULT_QUIZ, quiz_result
from bot.keyboards.common import offer_closed, quiz_kb
...
async def _ask(callback: CallbackQuery, index: int) -> None:
    ...
    header = f"Вопрос {index + 1} из {len(DEFAULT_QUIZ.questions)}\n\n{question.text}"
