...
- [ ] Notion: расшарить страницу интеграции → `/setup_notion <page_id>` → id в `.env` (`NOTION_DB_EPISODES`).
...
## 6. Наполнение контентом (владелец, в боте)
- [ ] `/add_season sweet | Сладкая империя | 299 | 179 | <архив> | 1` (текущий сезон).
- [ ] `/add_bonus …`, `/add_episode …` — по эпизодам (ключевое слово, теги).
...
- [ ] `/content` — проверить счётчики. (Для быстрого демо: `docker compose exec bot python -m scripts.seed_demo`.)
