...
- [ ] Notion: расшарить страницу интеграции → `/setup_notion <page_id>` → id в `.env` (`NOTION_DB_EPISODES`). База «Эпизоды» создастся со всеми полями (Ключевое слово, Теги, PDF-бонус, Аудио-бонус, Пост для Telegram). Синк Notion→БД идёт каждые 5 мин + `/sync_notion` вручную.
