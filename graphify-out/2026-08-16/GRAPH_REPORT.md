...
## Corpus Check
- 81 files · ~100,614 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 623 nodes · 1576 edges · 45 communities (44 shown, 1 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 37 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3c24f7a4`
...
1. `session_scope()` - 69 edges
2. `get_settings()` - 59 edges
3. `Settings` - 32 edges
4. `show()` - 26 edges
...
7. `User` - 15 edges
8. `Season` - 15 edges
9. `get_ambients()` - 15 edges
10. `Мастер-документ проекта «Алёхина без тормозов»` - 15 edges
...
Cohesion: 0.13
Nodes (27): BaseSettings, is_seen(), mark_seen(), Settings, date, MainProduct, ScheduledPost, announce_text() (+19 more)

### Community 11 - "get_settings"
Cohesion: 0.06
Nodes (89): BaseStorage, bonus_by_button(), deliver(), Any, callback_query, CallbackQuery, Message, buy_club() (+81 more)

### Community 12 - "session_scope"
Cohesion: 0.07
Nodes (67): async_sessionmaker, AsyncEngine, AsyncSession, Записать контакт в БД + Google Sheets. Один пользователь = одна строка (дедуп…, record(), _already_done(), fulfill(), _fulfill_all() (+59 more)
...
Cohesion: 0.18
Nodes (24): add_bonus(), add_episode(), add_product(), add_season(), admin_stats(), content_counts(), _is_admin(), Message (+16 more)
...
Cohesion: 0.19
Nodes (14): _auth_header(), confirmation_url(), create_payment(), get_payment(), Any, RuntimeError, YooKassaNotConfigured, get (+6 more)
...
Cohesion: 0.20
Nodes (9): PDF-гайд из текста, Гайд владельца — как пользоваться ботом, Готовый подкаст из аудио, Наполнение контентом (сезоны, бонусы, эпизоды, интенсивы), Настройка таблиц и Notion (разово, команды в боте), Пакеты магазина и цены, Подписка и триал (автоматически), Про большие файлы (+1 more)

### Community 25 - "catalog.py"
Cohesion: 0.37
Nodes (12): active_main_product(), _bonus(), bonus_by_id(), bonus_by_keyword(), current_season(), _episode(), _matches(), _product() (+4 more)
...
## Knowledge Gaps
- **71 isolated node(s):** `alehina-bot`, `Требования`, `GTK для WeasyPrint на Windows`, `Запуск`, `Аудио-пайплайн` (+66 more)
...
- **Why does `get_settings()` connect `get_settings` to `Settings`, `session_scope`, `producer.py`, `PipelineInputs`, `notion.py`, `admin.py`, `sheets.py`, `yookassa.py`, `catalog.py`?**
  _High betweenness centrality (0.139) - this node is a cross-community bridge._
- **Why does `session_scope()` connect `session_scope` to `users.py`, `Settings`, `get_settings`, `admin.py`, `catalog.py`?**
  _High betweenness centrality (0.076) - this node is a cross-community bridge._
- **Why does `Settings` connect `Settings` to `admin.py`, `get_settings`, `session_scope`, `producer.py`?**
