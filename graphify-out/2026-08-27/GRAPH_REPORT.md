# Graph Report - music bot  (2026-08-27)

## Corpus Check
- 87 files · ~77,709 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 715 nodes · 1879 edges · 46 communities (45 shown, 1 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 43 edges (avg confidence: 0.68)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `149eac64`
...
1. `session_scope()` - 76 edges
2. `get_settings()` - 68 edges
3. `Settings` - 42 edges
4. `show()` - 26 edges
5. `is_subscribed()` - 19 edges
6. `handle_incoming()` - 19 edges
7. `PipelineInputs` - 18 edges
8. `fulfill()` - 17 edges
9. `Base` - 17 edges
10. `process_episode()` - 16 edges
...
Cohesion: 0.16
Nodes (15): Два Telegram-канала (открытый публичный + закрытый частный), aiogram 3.x, BOT_TOKEN, CLOSED_CHANNEL_ID, Воронка продаж, Логика воронки: дешёвый гайд толкает в подписку, Монетизация, PDF-гайды 20-30 ₽ (+7 more)
...
Cohesion: 0.10
Nodes (36): _bonus_text(), _club_text(), _command(), handle_incoming(), main_keyboard(), _none_text(), _results_text(), _search_prompt() (+28 more)

### Community 10 - "Settings"
Cohesion: 0.16
Nodes (26): BaseSettings, publish_ready_posts(), publish_vk_posts(), Bot, is_seen(), mark_seen(), Settings, date (+18 more)

### Community 11 - "get_settings"
Cohesion: 0.06
Nodes (90): BaseStorage, deliver(), Any, Message, buy_club(), club_bonuses(), club_trial(), _offer() (+82 more)

### Community 12 - "session_scope"
Cohesion: 0.06
Nodes (64): async_sessionmaker, AsyncEngine, AsyncSession, add_bonus(), add_episode(), add_product(), add_season(), counts() (+56 more)

### Community 13 - "producer.py"
Cohesion: 0.10
Nodes (40): _week_stats(), week_stats_text(), choose_ambient(), _is_owner(), list_ambients(), owner_only(), owner_panel(), preview_ambient() (+32 more)
...
Cohesion: 0.17
Nodes (24): catalog_pages(), _client(), create_episodes_db(), _create_episodes_db_sync(), episode_id(), episode_status(), episode_title(), extract_catalog() (+16 more)
...
Cohesion: 0.18
Nodes (27): add_bonus(), add_episode(), add_product(), add_season(), admin_stats(), content_counts(), _is_admin(), Message (+19 more)

### Community 19 - "test_spec_features.py"
Cohesion: 0.15
Nodes (29): _already_done(), fulfill(), _fulfill_all(), _fulfill_season(), _fulfill_sub(), _fulfill_workbook(), _invite_link(), _notify() (+21 more)
...
Cohesion: 0.15
Nodes (24): bonus_by_button(), callback_query, CallbackQuery, active_main_product(), all_seasons(), _bonus(), bonus_by_id(), bonus_by_keyword() (+16 more)
...
- **Why does `get_settings()` connect `get_settings` to `test_vk.py`, `Settings`, `session_scope`, `producer.py`, `PipelineInputs`, `env.py`, `notion.py`, `admin.py`, `test_spec_features.py`, `catalog.py`?**
  _High betweenness centrality (0.162) - this node is a cross-community bridge._
- **Why does `session_scope()` connect `session_scope` to `test_vk.py`, `Settings`, `get_settings`, `producer.py`, `admin.py`, `test_spec_features.py`, `users.py`, `catalog.py`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `Settings` connect `Settings` to `test_vk.py`, `get_settings`, `session_scope`, `producer.py`, `admin.py`, `test_spec_features.py`, `catalog.py`?**
