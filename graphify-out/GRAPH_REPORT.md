...
## Graph Freshness
- Built from commit: `4878f4be`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Мастер-документ проекта «Алёхина без тормозов»
- Воронка и монетизация: каналы, бонусы, подписка
- Технический стек
- Оплаты Telegram-нативно (ключевое решение)
- Апселлы фаза 2+ (отдельная плата)
- Хранилища данных и надёжность
- Секреты / env-переменные (.env)
- Пошаговая инструкция по доступам для Юлии
- Люди и принципы проекта
- ВКонтакте: верх воронки
- Клиентские сценарии бота и настройки
- Пульт владельца: клавиатуры и эмбиенты
- Планировщик: рассылки, RSS, отзыв доступа
- Синк Notion → каталог бота
- Аудио-пайплайн ffmpeg: голос, музыка, дакинг
- PDF-гайды: рендер через WeasyPrint
- Клиент Notion: база «Эпизоды»
- README проекта и медиа
- Админ-команды: /admin, /setup_*, /add_*
- Пользователи: подписка, триал, купленные сезоны
- Google Sheets: контакты и продажи
- Деплой на VPS и настройка оплат
- Выдача доступа после оплаты (fulfillment)
- Логика бота (спека клиента, 14.08) — источник правды
- Гайд владельца — как пользоваться ботом
- Магазин: сезоны, тетради, пакет «все сезоны»
- Генератор эмбиентов (синтез фоновой музыки)
- Google Sheets — service-account JSON
- Метаданные пакета (pyproject)
- Антифлуд: ThrottlingMiddleware
- Слой БД: модели, сессия, контакты и продажи
- Тесты выдачи и идемпотентности платежей
- Подключение к БД и демо-данные
- Контент командами: сезоны, бонусы, эпизоды
- Тесты клуба и модель Season
- Тесты фазы 2: интенсив, финал сезона, все сезоны
- Тесты базовых сценариев: ключевое слово, поиск, покупка
...
### Community 1 - "Воронка и монетизация: каналы, бонусы, подписка"
Cohesion: 0.18
...
### Community 5 - "Хранилища данных и надёжность"
Cohesion: 0.25
...
### Community 8 - "Люди и принципы проекта"
Cohesion: 0.33
...
### Community 17 - "README проекта и медиа"
Cohesion: 0.11
...
### Community 22 - "Выдача доступа после оплаты (fulfillment)"
Cohesion: 0.33
...
### Community 45 - "Антифлуд: ThrottlingMiddleware"
Cohesion: 0.27
...
### Community 46 - "Слой БД: модели, сессия, контакты и продажи"
Cohesion: 0.22
...
### Community 47 - "Тесты выдачи и идемпотентности платежей"
Cohesion: 0.28
...
### Community 50 - "Тесты клуба и модель Season"
Cohesion: 0.26
...
### Community 51 - "Тесты фазы 2: интенсив, финал сезона, все сезоны"
Cohesion: 0.29
Nodes (5): MainProduct, FakeBot, test_active_intensive(), test_all_seasons_access(), test_finale_broadcast()

### Community 52 - "Тесты базовых сценариев: ключевое слово, поиск, покупка"
Cohesion: 0.43
...
- **Why does `get_settings()` connect `Клиентские сценарии бота и настройки` to `ВКонтакте: верх воронки`, `Пульт владельца: клавиатуры и эмбиенты`, `Планировщик: рассылки, RSS, отзыв доступа`, `Аудио-пайплайн ffmpeg: голос, музыка, дакинг`, `Слой БД: модели, сессия, контакты и продажи`, `Клиент Notion: база «Эпизоды»`, `Подключение к БД и демо-данные`, `Админ-команды: /admin, /setup_*, /add_*`, `Google Sheets: контакты и продажи`, `Выдача доступа после оплаты (fulfillment)`, `Магазин: сезоны, тетради, пакет «все сезоны»`?**
  _High betweenness centrality (0.148) - this node is a cross-community bridge._
- **Why does `session_scope()` connect `Пользователи: подписка, триал, купленные сезоны` to `ВКонтакте: верх воронки`, `Планировщик: рассылки, RSS, отзыв доступа`, `Синк Notion → каталог бота`, `Слой БД: модели, сессия, контакты и продажи`, `Тесты выдачи и идемпотентности платежей`, `Подключение к БД и демо-данные`, `Контент командами: сезоны, бонусы, эпизоды`, `Админ-команды: /admin, /setup_*, /add_*`, `Тесты клуба и модель Season`, `Тесты базовых сценариев: ключевое слово, поиск, покупка`, `Тесты фазы 2: интенсив, финал сезона, все сезоны`, `Выдача доступа после оплаты (fulfillment)`, `Магазин: сезоны, тетради, пакет «все сезоны»`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `Settings` connect `Планировщик: рассылки, RSS, отзыв доступа` to `ВКонтакте: верх воронки`, `Клиентские сценарии бота и настройки`, `Пульт владельца: клавиатуры и эмбиенты`, `Синк Notion → каталог бота`, `Админ-команды: /admin, /setup_*, /add_*`, `Тесты фазы 2: интенсив, финал сезона, все сезоны`, `Выдача доступа после оплаты (fulfillment)`?**
...
- **Should `ВКонтакте: верх воронки` be split into smaller, more focused modules?**
  _Cohesion score 0.12100840336134454 - nodes in this community are weakly interconnected._
- **Should `Клиентские сценарии бота и настройки` be split into smaller, more focused modules?**
